...
class TestRiskPolicy(TransactionCase):
    ...
    def test_partner_failed_sale_order_limit(self):
        ...
        err_msg = (
            "Error while validating constraint\n\n%s" % "Unauthorized sale order amount"
        )
        self.assertEqual(cm.exception.value, tools.ustr(err_msg))
        self.assertEqual(self.partner.sale_order_limit_policy, 100.00)
    ...
    def test_partner_success_sale_order_limit(self):
        ...
        self.assertEqual(self.partner.sale_order_limit_policy, 200.00)
